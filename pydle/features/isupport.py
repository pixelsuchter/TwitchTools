...
class ISUPPORTSupport(rfc1459.RFC1459Support):
    ...
    async def on_isupport_casemapping(self, value):
        ...
        if value in pydle.features.rfc1459.protocol.CASE_MAPPINGS:
            self._case_mapping = value
            self.channels = pydle.features.rfc1459.parsing.NormalizingDict(self.channels, case_mapping=value)
            self.users = pydle.features.rfc1459.parsing.NormalizingDict(self.users, case_mapping=value)
    ...
    async def on_isupport_chanmodes(self, value):
        ...
        if not pydle.features.rfc1459.protocol.BEHAVIOUR_LIST in self._channel_modes_behaviour:
            self._channel_modes_behaviour[pydle.features.rfc1459.protocol.BEHAVIOUR_LIST] = set()
        self._channel_modes_behaviour[pydle.features.rfc1459.protocol.BEHAVIOUR_LIST].update(list)

        if not pydle.features.rfc1459.protocol.BEHAVIOUR_PARAMETER in self._channel_modes_behaviour:
            self._channel_modes_behaviour[pydle.features.rfc1459.protocol.BEHAVIOUR_PARAMETER] = set()
        self._channel_modes_behaviour[pydle.features.rfc1459.protocol.BEHAVIOUR_PARAMETER].update(param)

        if not pydle.features.rfc1459.protocol.BEHAVIOUR_PARAMETER_ON_SET in self._channel_modes_behaviour:
            self._channel_modes_behaviour[pydle.features.rfc1459.protocol.BEHAVIOUR_PARAMETER_ON_SET] = set()
        self._channel_modes_behaviour[pydle.features.rfc1459.protocol.BEHAVIOUR_PARAMETER_ON_SET].update(param_set)

        if not pydle.features.rfc1459.protocol.BEHAVIOUR_NO_PARAMETER in self._channel_modes_behaviour:
            self._channel_modes_behaviour[pydle.features.rfc1459.protocol.BEHAVIOUR_NO_PARAMETER] = set()
        self._channel_modes_behaviour[pydle.features.rfc1459.protocol.BEHAVIOUR_NO_PARAMETER].update(noparams)
    ...
    async def on_isupport_excepts(self, value):
        ...
        self._channel_modes_behaviour[pydle.features.rfc1459.protocol.BEHAVIOUR_LIST].add(value)
    ...
    async def on_isupport_prefix(self, value):
        ...
        if not pydle.features.rfc1459.protocol.BEHAVIOUR_PARAMETER in self._channel_modes_behaviour:
            self._channel_modes_behaviour[pydle.features.rfc1459.protocol.BEHAVIOUR_PARAMETER] = set()
        self._channel_modes_behaviour[pydle.features.rfc1459.protocol.BEHAVIOUR_PARAMETER].update(set(modes))

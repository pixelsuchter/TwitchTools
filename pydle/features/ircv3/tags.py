...
class TaggedMessage(rfc1459.RFC1459Message):
    ...
    def parse(cls, line, encoding=pydle.protocol.DEFAULT_ENCODING):
        ...
        if message.endswith(pydle.features.rfc1459.protocol.LINE_SEPARATOR):
            message = message[:-len(pydle.features.rfc1459.protocol.LINE_SEPARATOR)]
        elif message.endswith(pydle.features.rfc1459.protocol.MINIMAL_LINE_SEPARATOR):
            message = message[:-len(pydle.features.rfc1459.protocol.MINIMAL_LINE_SEPARATOR)]
        raw = message
...
class TaggedMessageSupport(rfc1459.RFC1459Support):
    ...
    def _parse_message(self):
        sep = pydle.features.rfc1459.protocol.MINIMAL_LINE_SEPARATOR.encode(self.encoding)
